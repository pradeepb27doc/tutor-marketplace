...
import { searchApiClient } from "../../features/search/services/search-service";
import type {
  TutorCard,
  TutorSortKey,
  SearchTutorsParams,
...
const SORT_OPTIONS: { label: string; value: TutorSortKey }[] = [
...
function getBadge(tutor: TutorCard): { label: string; icon: typeof ShieldCheck } | null {
  if (tutor.isVerified) return { label: "Verified", icon: ShieldCheck };
  if (tutor.reviewCount > 200) return { label: "Top rated", icon: Star };
  if (tutor.completedClassesCount > 100) return { label: "Experienced", icon: Award };
  return null;
}
...
export default function SearchPage() {
  ...
  // totalCount is tracked via tutors.length
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !isLoadingMore && !isLoading) {
          const params: SearchTutorsParams = {
            ...searchParams,
            sort,
            cursor: nextCursor,
            limit: 20,
          };
          fetchTutors(params, true);
        }
      },
      { threshold: 0.1 },
    );
  ...
  }, [hasMore, isLoadingMore, isLoading, nextCursor, searchParams, sort, fetchTutors]);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const params: SearchTutorsParams = { sort, limit: 20 };

    if (subjectInput.trim()) {
      params.subjectSlug = subjectInput.trim().toLowerCase();
    }
    if (boardInput.trim()) {
      params.curriculum = boardInput.trim();
    }
    if (locationInput.trim()) {
      const loc = locationInput.trim().toLowerCase();
      if (loc === "online") {
        params.serviceMode = "ONLINE";
      } else {
        params.city = loc;
      }
    }
  ...
  const handleSortChange = (newSort: TutorSortKey) => {
    setSort(newSort);
    const params = { ...searchParams, sort: newSort, limit: 20 };
    fetchTutors(params);
  };

  const handleRetry = () => {
    fetchTutors({ ...searchParams, sort, limit: 20 });
  };
  ...
  const handleClearFilters = () => {
    ...
    setSearchParams({});
    setSort("RATING");
    fetchTutors({ sort: "RATING", limit: 20 });
  };
      ...
      <header className="sticky top-0 z-50 border-b border-border/70 bg-background/82 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="#results"
              className="rounded-full bg-foreground px-5 py-2.5 text-sm font-semibold text-background transition-transform duration-300 hover:-translate-y-0.5"
            >
              View Tutors
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-5 max-w-3xl text-5xl font-semibold tracking-[-0.06em] sm:text-6xl lg:text-7xl">
                Find tutors who fit your learning rhythm.
              </h1>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-foreground/58">
                Compare verified educators by subject, board, price, rating, availability, and
                learning mode.
              </p>
            </div>
            <div className="grid gap-3 rounded-[2rem] border border-border bg-secondary/35 p-4 sm:grid-cols-3 lg:min-w-[28rem]">
              {[
                ["4.9★", "Avg rating"],
                [String(tutors.length || "0"), "Tutors loaded"],
                ["Today", "Earliest slot"],
              ].map(([value, label]) => (
          ...
          <form
            ...
            className="mt-8 rounded-[2rem] border border-border bg-background p-3 shadow-[0_24px_80px_rgba(15,23,42,0.08)]"
                ...
                <div className="grid gap-5 xl:grid-cols-2">
                  {tutors.map((tutor) => {
                    ...
                    return (
                      <article
                        key={tutor.id}
                        className="group rounded-[2rem] border border-border bg-background p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_28px_90px_rgba(15,23,42,0.10)]"
                      >
                        <div className="flex gap-4">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-5 grid gap-3 text-sm text-foreground/58 sm:grid-cols-2">
                              ...
                              <span className="flex items-center gap-2">
                                <CalendarDays className="size-4 text-foreground" aria-hidden="true" />{" "}
                            ...
                            <div className="mt-6 flex flex-col gap-3 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between">
                              <div>
                                ...
                                <p className="mt-1 text-2xl font-semibold tracking-[-0.04em] text-foreground">
                                  {formatFee(tutor.lowestHourlyRate ?? tutor.baseHourlyRate, tutor.currency)}
                                </p>
                              </div>
